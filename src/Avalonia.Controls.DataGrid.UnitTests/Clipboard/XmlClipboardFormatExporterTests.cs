// Copyright (c) Wiesław Šoltés. All rights reserved.
// Licensed under the MIT license. See LICENSE file in the project root for details.

using Avalonia.Controls;
using Avalonia.Headless.XUnit;
using Avalonia.Input;
using Xunit;

namespace Avalonia.Controls.DataGridTests.Clipboard;

public class XmlClipboardFormatExporterTests
{
    [AvaloniaFact]
    public void XmlExporter_Writes_Xml()
    {
        var rows = ClipboardTestData.BuildRows();
        var item = new DataTransferItem();
        var exporter = new XmlClipboardFormatExporter();

        var result = exporter.TryExport(
            new DataGridClipboardExportContext(
                new DataGrid(),
                rows,
                DataGridClipboardCopyMode.IncludeHeader,
                DataGridClipboardExportFormat.Xml,
                DataGridSelectionUnit.FullRow),
            item);

        Assert.True(result);
        var xml = item.TryGetRaw(XmlClipboardFormatExporter.XmlFormat) as string;
        Assert.NotNull(xml);
        Assert.Contains("<rows>", xml);
        Assert.Contains("<header>", xml);
        Assert.Contains("<row>", xml);
    }
}
